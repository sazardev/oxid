...
- `OXID_ADDR` default `0.0.0.0:8080`, `OXID_GC_INTERVAL_SECS` default `30`, `OXID_DEPLOY_CONCURRENCY` — queued deploys per drain wave; **unset it and `drain_width()` derives it from the fleet** (4 per placement-accepting node, capped 32). Set pins it. Per-project `git fetch` is coalesced in `service/refresh_coalescer.rs` — sharing is decided on when a caller *asked*, never on the age of the result, so it is coalescing rather than caching and has no staleness window
