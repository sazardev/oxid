...
(function () {
  ...
  const CATALOG = {
    en: {
      ...
      "project.workspaceTitle": "Services in this workspace",
      "project.workspaceHint": "A {kind} workspace (task runner: {runner}). Each of these is deployable on its own; the one marked below is what this project builds.",
      "project.workspaceActive": "building",
      "project.workspaceSwitch": "To deploy a different one, set [build].context in oxid.toml to its path.",
      "project.stackTitle": "Detected from {evidence}",
      "project.stackDetected": "Detected from {evidence} ({confidence}). Oxid generates a Dockerfile for this at build time; commit one of your own and it takes over.",
      "project.confidence.certain": "certain",
      "project.confidence.likely": "likely",
      "a11y.skipToContent": "Skip to content",
      "a11y.mainNav": "Main",
      "notice.projectAdded": "Project `{name}` registered. Push a branch, or deploy one from its page.",
      "nav.diagnostics": "Diagnostics",
      "action.cancelDeploy": "cancel",
      "confirm.cancelDeploy": "Drop the queued deploy of `{branch}`? The push is discarded — it is not deployed later.",
      "notice.deployCancelled": "Queued deploy of `{branch}` cancelled.",
      "notice.cancelFailed": "Could not cancel: {error}",
      "queue.hint": "Pushes waiting for host capacity, oldest first. The queue stops at the first entry that does not fit, so a large deploy is never starved by smaller ones behind it — which also means one entry that can never fit holds up the rest.",
      "queue.col.attempts": "attempts",
      "diag.title": "Diagnostics",
      "diag.hint": "Everything you would run `oxid doctor` for, plus what only the daemon can see.",
      "diag.daemonTitle": "Daemon",
      "diag.reachable": "reachable",
      "diag.version": "version",
      "diag.latency": "latency",
      "diag.checkedAt": "checked",
      "diag.yes": "yes",
      "diag.no": "no",
      "diag.credentialTitle": "Credential",
      "diag.tokenValid": "authenticates",
      "diag.tokenScope": "scope",
      "diag.scopeMaster": "master — full access",
      "diag.scopeNamed": "named token — cannot manage tokens",
      "diag.tokenHint": "Set a token in the field at the top right. A fresh install can fetch one on the daemon's own host with `oxid token generate`.",
      "env.primaryService": "(public)",
      "diag.fleetTitle": "Fleet",
      "diag.fleetHint": "One control plane, several nodes. A node marked down keeps its environments — nothing is moved or destroyed because a machine stopped answering.",
      "diag.col.node": "NODE",
      "diag.col.nodeState": "STATE",
      "diag.col.nodeMemory": "MEMORY",
      "diag.col.nodeUsed": "COMMITTED",
      "diag.col.nodeEnvs": "ENVS",
      "diag.nodeDisconnected": "registered, but this daemon cannot reach it",
      "diag.capacityTitle": "Capacity",
      "diag.hostMemory": "host",
      "diag.running": "running now",
      "diag.queued": "queued",
      "diag.seeQueue": "see the queue",
      "diag.routing": "routing",
      "diag.viaTraefik": "via Traefik",
      "diag.directPublish": "direct publish",
      "diag.directPublishHint": "each environment publishes a host port; scale-to-zero is off, since nothing refreshes last-seen",
      "diag.goAdmin": "repair it in Admin",
      "env.selected.one": "1 selected",
      "env.selected.other": "{count} selected",
      "confirm.bulk.pause.one": "Suspend 1 environment? It stops its container; the next request wakes it.",
      "confirm.bulk.pause.other": "Suspend {count} environments? Each stops its container; the next request wakes it.",
      "confirm.bulk.wake.one": "Wake 1 environment?",
      "confirm.bulk.wake.other": "Wake {count} environments?",
      "confirm.bulk.destroy.one": "Destroy 1 environment permanently? This cannot be undone.",
      "confirm.bulk.destroy.other": "Destroy {count} environments permanently? This cannot be undone.",
      "notice.bulkDone.one": "{action}: 1 environment.",
      "notice.bulkDone.other": "{action}: {count} environments.",
      "action.register": "register",
      "action.working": "working…",
      "action.recheck": "re-check",
      "project.registerTitle": "Register a project",
      "project.registerHint": "Point Oxid at a git repository. Every branch pushed to it can then get its own environment.",
      "project.registerModeUrl": "clone URL",
      "project.registerModeDir": "path on the daemon",
      "project.registerUrlPlaceholder": "https://github.com/org/repo.git",
      "project.registerDirPlaceholder": "/repos/my-app",
      "project.registerTokenPlaceholder": "git token (private repos)",
      "env.clearSelection": "clear selection",
      "env.selectAll": "Select every environment shown",
      "env.selectOne": "Select {branch}",
      "notice.bulkPartial": "{done} of {count} succeeded. Failed: {failed}",
      "notice.infraRepaired": "Infrastructure repaired.",
      "notice.restoreStaged": "Restore staged — it is applied the next time the daemon restarts.",
      "notice.restoreFailed": "Restore failed: {error}",
      "admin.infraTitle": "Infrastructure",
      "admin.infraHint": "The Docker network and Traefik wiring that scale-to-zero and wake-on-request depend on.",
      "admin.infraLoading": "Checking…",
      "admin.infraNetwork": "network",
      "admin.infraTraefik": "traefik",
      "admin.infraPort": "published on port {port}",
      "admin.infraWake": "wake-on-request",
      "admin.infraWakeHint": "this daemon's own container labels",
      "admin.infraOk": "ok",
      "admin.infraMissing": "missing",
      "admin.infraRepair": "create what's missing",
      "admin.infraAllGood": "Everything wake-on-request needs is in place.",
      "admin.backupTitle": "Backup and restore",
      "admin.backupHint": "A snapshot holds the database and the key that encrypts every secret. Keep it somewhere you would keep a password.",
      "admin.restoreBackup": "restore from file…",
      "admin.restoreHint": "A restore is staged, not applied live: the daemon swaps the database in on its next restart, keeping the one it replaces.",
      "admin.restoreConfirm": "Restore from `{name}`? This replaces the database and every secret on the next daemon restart.",
    ...
    es: {
      ...
      "project.workspaceTitle": "Servicios en este workspace",
      "project.workspaceHint": "Un workspace {kind} (orquestador: {runner}). Cada uno es desplegable por su cuenta; el marcado abajo es el que construye este proyecto.",
      "project.workspaceActive": "construyendo",
      "project.workspaceSwitch": "Para desplegar otro, pon su ruta en [build].context dentro de oxid.toml.",
      "project.stackTitle": "Detectado a partir de {evidence}",
      "project.stackDetected": "Detectado a partir de {evidence} ({confidence}). Oxid genera un Dockerfile para esto al construir; si añades el tuyo, manda el tuyo.",
      "project.confidence.certain": "seguro",
      "project.confidence.likely": "probable",
      "a11y.skipToContent": "Saltar al contenido",
      "a11y.mainNav": "Principal",
      "notice.projectAdded": "Proyecto `{name}` registrado. Empuja una rama, o despliega una desde su página.",
      "nav.diagnostics": "Diagnóstico",
      "action.cancelDeploy": "cancelar",
      "confirm.cancelDeploy": "¿Descartar el despliegue en cola de `{branch}`? El push se descarta — no se despliega más tarde.",
      "notice.deployCancelled": "Despliegue en cola de `{branch}` cancelado.",
      "notice.cancelFailed": "No se pudo cancelar: {error}",
      "queue.hint": "Pushes esperando capacidad del host, del más antiguo al más nuevo. La cola se detiene en la primera entrada que no cabe, para que un despliegue grande nunca quede postergado por otros más pequeños detrás — lo que también significa que una entrada que jamás cabrá retiene al resto.",
      "queue.col.attempts": "intentos",
      "diag.title": "Diagnóstico",
      "diag.hint": "Todo aquello para lo que ejecutarías `oxid doctor`, más lo que solo el daemon puede ver.",
      "diag.daemonTitle": "Daemon",
      "diag.reachable": "accesible",
      "diag.version": "versión",
      "diag.latency": "latencia",
      "diag.checkedAt": "comprobado",
      "diag.yes": "sí",
      "diag.no": "no",
      "diag.credentialTitle": "Credencial",
      "diag.tokenValid": "autentica",
      "diag.tokenScope": "alcance",
      "diag.scopeMaster": "maestra — acceso completo",
      "diag.scopeNamed": "token con nombre — no puede gestionar tokens",
      "diag.tokenHint": "Pon un token en el campo de arriba a la derecha. Una instalación nueva puede obtenerlo, en el propio host del daemon, con `oxid token generate`.",
      "env.primaryService": "(público)",
      "diag.fleetTitle": "Flota",
      "diag.fleetHint": "Un control plane, varios nodos. Un nodo marcado como caído conserva sus entornos — nada se mueve ni se destruye porque una máquina deje de responder.",
      "diag.col.node": "NODO",
      "diag.col.nodeState": "ESTADO",
      "diag.col.nodeMemory": "MEMORIA",
      "diag.col.nodeUsed": "COMPROMETIDA",
      "diag.col.nodeEnvs": "ENTORNOS",
      "diag.nodeDisconnected": "registrado, pero este daemon no puede alcanzarlo",
      "diag.capacityTitle": "Capacidad",
      "diag.hostMemory": "host",
      "diag.running": "en ejecución",
      "diag.queued": "en cola",
      "diag.seeQueue": "ver la cola",
      "diag.routing": "enrutado",
      "diag.viaTraefik": "vía Traefik",
      "diag.directPublish": "publicación directa",
      "diag.directPublishHint": "cada entorno publica un puerto del host; el escalado a cero queda apagado, porque nada refresca la última visita",
      "diag.goAdmin": "repararlo en Admin",
      "env.selected.one": "1 seleccionado",
      "env.selected.other": "{count} seleccionados",
      "confirm.bulk.pause.one": "¿Suspender 1 entorno? Detiene su contenedor; la siguiente petición lo despierta.",
      "confirm.bulk.pause.other": "¿Suspender {count} entornos? Cada uno detiene su contenedor; la siguiente petición lo despierta.",
      "confirm.bulk.wake.one": "¿Despertar 1 entorno?",
      "confirm.bulk.wake.other": "¿Despertar {count} entornos?",
      "confirm.bulk.destroy.one": "¿Destruir 1 entorno de forma permanente? No se puede deshacer.",
      "confirm.bulk.destroy.other": "¿Destruir {count} entornos de forma permanente? No se puede deshacer.",
      "notice.bulkDone.one": "{action}: 1 entorno.",
      "notice.bulkDone.other": "{action}: {count} entornos.",
      "action.register": "registrar",
      "action.working": "trabajando…",
      "action.recheck": "volver a comprobar",
      "project.registerTitle": "Registrar un proyecto",
      "project.registerHint": "Apunta Oxid a un repositorio git. Cada rama que se empuje podrá tener su propio entorno.",
      "project.registerModeUrl": "URL de clonado",
      "project.registerModeDir": "ruta en el daemon",
      "project.registerUrlPlaceholder": "https://github.com/org/repo.git",
      "project.registerDirPlaceholder": "/repos/mi-app",
      "project.registerTokenPlaceholder": "token git (repos privados)",
      "env.clearSelection": "quitar selección",
      "env.selectAll": "Seleccionar todos los entornos mostrados",
      "env.selectOne": "Seleccionar {branch}",
      "notice.bulkPartial": "{done} de {count} salieron bien. Fallaron: {failed}",
      "notice.infraRepaired": "Infraestructura reparada.",
      "notice.restoreStaged": "Restauración preparada — se aplica la próxima vez que arranque el daemon.",
      "notice.restoreFailed": "La restauración falló: {error}",
      "admin.infraTitle": "Infraestructura",
      "admin.infraHint": "La red de Docker y el cableado de Traefik de los que dependen el escalado a cero y el despertar por petición.",
      "admin.infraLoading": "Comprobando…",
      "admin.infraNetwork": "red",
      "admin.infraTraefik": "traefik",
      "admin.infraPort": "publicado en el puerto {port}",
      "admin.infraWake": "despertar por petición",
      "admin.infraWakeHint": "etiquetas del contenedor de este daemon",
      "admin.infraOk": "correcto",
      "admin.infraMissing": "falta",
      "admin.infraRepair": "crear lo que falta",
      "admin.infraAllGood": "Está todo lo que el despertar por petición necesita.",
      "admin.backupTitle": "Copia de seguridad y restauración",
      "admin.backupHint": "Una copia contiene la base de datos y la clave que cifra todos los secretos. Guárdala donde guardarías una contraseña.",
      "admin.restoreBackup": "restaurar desde archivo…",
      "admin.restoreHint": "La restauración se prepara, no se aplica en caliente: el daemon cambia la base de datos en su próximo arranque y conserva la que sustituye.",
      "admin.restoreConfirm": "¿Restaurar desde `{name}`? Esto reemplaza la base de datos y todos los secretos en el próximo arranque del daemon.",
