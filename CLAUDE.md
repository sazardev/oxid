...
- `OXID_DEPLOY_CONCURRENCY` — how many queued deploys one drain wave runs at once. **Unset it and Oxid works it out from the fleet**: 4 per node that accepts placements, capped at 32. A constant was wrong in both directions once there was more than one machine — 4 across five nodes leaves four fifths idle, and 4 across one full node is three wasted admission round trips and three requeues. Draining and down nodes are excluded, since a wave sized for hardware that refuses placements mostly comes back unplaced. Setting it pins the number: an operator who chose one meant it. Measured on 15 simultaneous pushes to one node: 7.1s at 4, 4.2s at 16
...
- `OXID_NODE_STATUS_TIMEOUT_SECS` (default 5) — deadline on a *status* query to a node (`docker info`, container status), far shorter than the connection's own timeout, which has to fit a build. A partitioned node blackholes rather than refusing, so without this a deploy aimed at a healthy node waits out the kernel: measured at 121s, 7s after. Raise it for a high-latency link
